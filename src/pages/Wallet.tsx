...
import { useFetchGroups } from "@/hooks/groups/use-fetch-group";
import { useFetchMembers } from "@/hooks/groups/use-fetch-members";
...
const Wallet = () => {
  ...
  const { data: groups, isLoading: isLoadingGroups, isError: isErrorGroups } = useFetchGroups();
  const { data: members, isLoading: isLoadingMembers, isError:isErrorMembers } = useFetchMembers();
  
  // TODO
  // Replace common div with loading animation component
  if (isLoadingGroups || isLoadingMembers) return <div>Loading...</div>
if (isErrorGroups || isErrorMembers) return <div>Oops... something went wrong{isErrorGroups ?? isErrorMembers}</div>
