...
const NotificationsPage = () => {
  ...
  // ← new: track per-notification errors
  ...
  const handleAccept = (
    ...
    setErrorId(null);
    acceptInvite(
      { groupId, notificationId, groupType },
      {
        onSuccess: () => {
        ...
        onError: (err: any) => {
          ...
          // surface the RPC error message (e.g. "Insufficient balance")
  ...
  // ← FIXED: also hide notifications that were acted on locally
  // (covers the gap between delete and refetch)
  const visibleNotifications = notifications.filter((n) => !actedOn.has(n.id));
  return (
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      {isInvite && n.groupId && !hasActed && (
                        <div className="mt-3 space-y-2">
                          {/* Show share amount for BILL invites */}
                          {n.group?.type === "BILL" && (
                            <p className="text-xs text-muted-foreground">
                              Your share will be locked from your wallet when
                              you accept.
                            </p>
                          )}
